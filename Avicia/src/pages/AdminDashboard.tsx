// AdminDashboard version 9 - Modificado
...
const AdminDashboard = () => {
  const navigate = useNavigate(); // <<< 2. INSTANCIADO
  ...
  const handleSaveUser = async () => {
    if (!formData.name || !formData.email || !formData.role) {
      toast({
        title: "Erro",
        description:
          "Preencha todos os campos obrigatórios (Nome, Email, Função).",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!editingUser) {
      if (
        !dataToSave.password ||
        dataToSave.password !== dataToSave.confirmPassword
      ) {
        toast({
          title: "Erro de Senha",
          description: "As senhas são obrigatórias e devem coincidir.",
          variant: "destructive",
        });
        return;
      }
    ...
    if (
      ...
      try {
        ...
        const nome =
          ultimoEspaco > 0 ? nomeCompleto.substring(0, ultimoEspaco) : nomeCompleto;
        const sobrenome =
        ...
        if (formData.role === "Paciente") {
          ...
          if (!formData.cargo || !formData.setor || !formData.matricula) {
            toast({
              title: "Erro",
              description: "Cargo, Setor e Matrícula são obrigatórios.",
              variant: "destructive",
            });
            setLoading(false);
            return;
          ...
          if (
            ...
            toast({
              title: "Erro",
              description:
                "Campos obrigatórios faltando para Profissional de Saúde.",
              variant: "destructive",
            });
            setLoading(false);
            return;
      ...
      try {
        setUsers(
          users.map((u) =>
            u.id === editingUser.id ? ({ ...u, ...dataToSave } as User) : u
          )
        );
        toast({
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4 max-h-[70vh] overflow-y-auto pr-6">
          ...
          {formData.role === "Paciente" && (
            <div className="space-y-2 col-span-1 md:col-span-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({ ...formData, preferencia_contato: value })
                }
              >
                <SelectTrigger>
          ...
          {formData.role === "Profissional de Saúde" && (
            <>
              ...
              <div className="space-y-2">
                <Label htmlFor="registroConselho">Nº do Conselho</Label>
                <Input
                  id="registroConselho"
                  value={formData.registroConselho || ""}
                  onChange={(e) =>
                    setFormData({ ...formData, registroConselho: e.target.value })
                  }
      ...
      <CardContent>
        <div className="space-y-4">
          <p className="text-gray-600">
            Gerencie os perfis de usuário (roles) e defina permissões detalhadas para
            cada módulo do sistema. Clique no botão abaixo para acessar a tela de
            gerenciamento.
          </p>
          <Button
            onClick={() => navigate("/admin/role-registration")} // Ajuste esta rota conforme necessário
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-gray-700">
                      <Activity className="h-5 w-5" /> Atividades Recentes do
                      Sistema
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
