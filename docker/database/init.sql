...
-- ======== ENUM TYPES ============
-- ================================
CREATE TYPE tipo_log_auditoria AS ENUM ('MODIFICAÇÃO', 'CRIAÇÃO', 'EXCLUSÃO', 'AVISO', 'ERRO');
...
CREATE TABLE cadastro.usuario_paciente (
  ...
  id_usuario int REFERENCES seguranca.usuario(id_usuario) ON DELETE CASCADE,
...
CREATE TABLE cadastro.usuario_profissional_saude (
  ...
  id_usuario int NOT NULL REFERENCES seguranca.usuario(id_usuario) ON DELETE CASCADE,
  matricula varchar UNIQUE NOT NULL,
  conselho varchar NOT NULL,
  registroconselho varchar UNIQUE NOT NULL,
...
-- ================================
CREATE TABLE seguranca.paciente_profissional_saude (
    id_profissional_saude INT NOT NULL REFERENCES cadastro.usuario_profissional_saude(id_profissional_saude) ON DELETE CASCADE,
    id_paciente INT NOT NULL REFERENCES cadastro.usuario_paciente(id_paciente) ON DELETE CASCADE,
    data_vinculo TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id_profissional_saude, id_paciente)
);

CREATE TABLE seguranca.paciente_funcionario (
    id_funcionario INT NOT NULL REFERENCES cadastro.usuario_funcionario(id_funcionario) ON DELETE CASCADE,
    id_paciente INT NOT NULL REFERENCES cadastro.usuario_paciente(id_paciente) ON DELETE CASCADE,
    data_vinculo TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id_funcionario, id_paciente)
);

CREATE TABLE seguranca.paciente_usuario (
    id_usuario INT NOT NULL REFERENCES seguranca.usuario(id_usuario) ON DELETE CASCADE,
    id_paciente INT NOT NULL REFERENCES cadastro.usuario_paciente(id_paciente) ON DELETE CASCADE,
    data_vinculo TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE seguranca.log_auditoria (
  ...
  tipo_log tipo_log_auditoria NOT NULL
);
